...
const ProductsComp = () => { 
    ...
    const handleSearchResult = (e) => {
        ...
        const result = Products?.filter((product) =>
            ...
            product?.list?.toLowerCase().includes(searchText.toLowerCase()) ||
            product?.instructor?.toLowerCase().includes(searchText.toLowerCase()) ||
            product?.language?.toLowerCase().includes(searchText.toLowerCase())
        );
        setCategoryProduct([]);
        setCheckboxFilter([]);
        setReviewFilter([]);
    ...
    const handleReviewFilter = (num) => {
        if (num) {
            const filterData = Products?.filter((cData) => Math.ceil(cData?.rating?.total_rating / cData?.rating?.total_people) === parseInt(num));
            setSearch([]);
            setCategoryProduct([]);
            setCheckboxFilter([]);
